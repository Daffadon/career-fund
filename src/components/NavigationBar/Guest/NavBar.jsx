      ...
      {width <= 870 ? (
        <nav className="p-3 border-gray-200 rounded  dark:bg-gray-800 dark:border-gray-700">
          <div className="container flex flex-wrap items-center justify-between mx-auto">
            <button
              ...
              <svg
                className="w-10 h-10 right-10 fixed"
                aria-hidden="true"
                fill="currentColor"
                viewBox="0 0 20 20"
              ...
              <div className="fixed top-24 right-5 bg-white border-none border-2 w-[16rem] h-[20vh] rounded-xl">
                <div className="h-full flex flex-col items-center justify-evenly">
                  ...
                  {localStorage.getItem('token') ? (
                    <>
                      <div
                        className="flex gap-3 order-1 items-center"
                        onClick={() => {
                        ...
                        <div>
                          <p className={`${fontType['h3']}`}>
                            {user.data.user.name.split(' ').length >= 3
                              ? `${user.data.user.name.split(' ')[0]} ${
                                  user.data.user.name.split(' ')[1]
                                }`
                              : user.data.user.name}
                          </p>
                          <p className={`${fontType['p1']} text-neutral30`}>
                            {user.data.user.city && user.data.user.region
                              ? `${user.data.user.city}, ${user.data.user.region}`
                              : '-'}
                          </p>
                        </div>
                      ...
                      {showAccount && <AccountLogout navigate={'/account'} lite={true} />}
                    </>
                  ) : (
                    <>
                      <button>Set Token to view auth mode</button>
                      <Link
                        className={`${fontType['button']} bg-secondary50 py-2 px-8 rounded-full hover:scale-150`}
                        to="/login">
                        Masuk / Daftar
                      </Link>
                    </>
        ...
        <>
          ...
          {localStorage.getItem('token') ? (
            <>
              <div
                className="flex gap-3 px-8 mr-10 cursor-pointer justify-center items-center"
                onClick={() => {
                  setShowAccount(!showAccount);
                }}>
                {/* {user.data.user.url_icon ?
              ...
              {showAccount && <AccountLogout navigate={'/account'} lite={false} />}
            </>
          ) : (
            <div className="flex justify-center items-center">
              {localStorage.getItem('token') ? (
                <button
                  onClick={unsetToken}
                  className="mr-5 bg-blue-500 px-5 py-2 rounded-xl text-white">
                  Unset Token
                </button>
              ) : (
                <button
              ...
              <Link
                className={`${fontType['button']} cursor-pointer bg-secondary50 mr-10 py-2 px-8 rounded-full`}
                to="/login">
                Masuk / Daftar
              </Link>
            </div>

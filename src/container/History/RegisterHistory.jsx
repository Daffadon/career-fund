          ...
          <div key={item.key} className="bg-white w-10/12 lg:w-5/12 min-h-[20vh] rounded-3xl px-7 py-9 flex flex-col gap-8 shadow-[5px_5px_10px_rgba(0,0,0,0.05)]  ">
            ...
            <div className="flex flex-col xl:flex-row justify-between">
              ...
              <div className="flex gap-2 mt-3">
                {item.education.map((tag, index) => {
                  return (
                    <p key={index} className="bg-warning10 text-warning90 py-1 px-5 rounded-full  h-max">
                      {tag}
                    </p>
                  );
                })}
            ...
            <p className={`${fontType["p1"]} text-neutral50 text-justify w-11/12`}>{item.deskripsi}</p>
          </div>
        )
      })}
    </>
